...
namespace Volt
{
	...
	struct Vector2i
	{
		...
		Vector2i(uint32_t new_x, uint32_t new_y)
		{
			x = new_x;
			y = new_y;
		}
	...
	struct Vector3
	{
		...
		// Constructs a vector3 with an x, y, and z coordinate
	...
	struct Vector3i
	{
		...
		Vector3i(uint32_t new_x, uint32_t new_y, uint32_t new_z)
		{
			x = new_x;
			y = new_y;
			z = new_z;
	...
	struct Vector4
	{
		...
		// Constructs a vector4 with an x, y, z and w coordinate
	...
	struct Vector4i
	{
		...
		Vector4i(uint32_t new_x, uint32_t new_y, uint32_t new_z, uint32_t new_w)
		{
			x = new_x;
			y = new_y;
			z = new_z;
	...
	struct Matrix22
	{
		...
		Matrix22 Translate(Vector2 translate);		// Translate a 4x4 matrix by a Vector4.

		Matrix22 Scale(const Matrix22& other);		// Scale a 2x2 matrix.
		Matrix22 Scale(Vector2 scale);				// Scale a 2x2 matrix by a vector2.
	...
	struct Matrix33
	{
		...
		Matrix33 Translate(Vector3 translate);		// Translate a 4x4 matrix by a Vector4.

		Matrix33 Scale(const Matrix33& other);		// Scale a 3x3 matrix.
		Matrix33 Scale(Vector3 scale);				// Scale a 3x3 matrix by a vector2.

		Matrix33 Rotate(float radians, const Matrix33& other);		// Rotate a 3x3 matrix.
	...
	struct Matrix44
	{
		...
		Matrix44 Translate(const Matrix44& other);	// Translate a 4x4 matrix.
		Matrix44 Translate(Vector4 translate);		// Translate a 4x4 matrix by a Vector4.
		Matrix44 Scale(const Matrix44& other);		// Scale a 4x4 matrix.
		Matrix44 Scale(Vector4 scale);				// Scale a 4x4 matrix by a vector2.

		Matrix44 Rotate(float radians, const Matrix44& other);		// Rotate a 3x3 matrix.
